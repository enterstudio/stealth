...
stealth._handleAction = function(action) {
  ...
  router.register(action.path, 'post', action.middleware || [ ], function(request, res) {
    action.action(request, function(err, actionData) {
      if (err) return router.handleError(res, err, true);
