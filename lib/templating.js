...
templating._loadTemplatesFromFolder = function(basePath, path) {
  fs.readdirSync(basePath + path).forEach(function(file) {
    ...
    if (file.match(/\.hbs$/i)) { // template
      ...
      if (file.indexOf('_') === 0) {
        filePath = filePath.replace(/^\//, '').replace(/\.hbs$/, '')
        debug.templateLoad("Registering helper", filePath);
        handlebars.registerPartial(filePath, template)
      }
    } else { // folder
      var someFolder = { };
...
templating.wrapInScript = function(script) {
  return "<script type='text/javascript'>" + script + "</script>";
};

var clientsideStealth = fs.readFileSync(__dirname + "/clientsideStealth.js").toString();
templating.getClientSideStealth = function() {
  return templating.wrapInScript(clientsideStealth);
};
