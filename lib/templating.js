...
templating._buildFullTemplatePath = function(request, template) {
  var requestedDomain = request.route.host;
...
templating.wrapInDiv = function(id, markup) {
  return "<div id='" + id + "'>" + markup + "</div>";
};

templating._loadScript = function(path) {
  var script = fs.readFileSync(path).toString();
  return "<script type='text/javascript'>" + script + "</script>";
};

templating.getClientSideStealth = function() {
  var jQuery = templating._loadScript(__dirname + "/jquery-1.12.3.min.js");
  var clientsideStealth = templating._loadScript(__dirname + "/clientsideStealth.js");
  return jQuery + clientsideStealth;
};
